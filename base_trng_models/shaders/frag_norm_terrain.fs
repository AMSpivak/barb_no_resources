...
void main()
{
    ...
    vec3 normal_bump_x = normalize((texture(NormalTexture, TexCoord_X).xyz));//* 2.0 - vec3 (1.0,1.0,1.0));
    vec3 normal_bump_y = normalize((texture(NormalTexture, TexCoord_Y).xyz)* 2.0 - vec3 (1.0,1.0,1.0));
    vec3 normal_bump_z = normalize((texture(NormalTexture, TexCoord_Z).xyz)* 2.0 - vec3 (1.0,1.0,1.0));


    normal_bump_y = vec3(normal_bump_y.x,normal_bump_y.z,-normal_bump_y.y);
    normal_bump_z = vec3(normal_bump_z.x,-normal_bump_z.y,normal_bump_z.z);
    //normal_bump_x = vec3(normal_bump_x.z * mx, normal_bump_x.y* mx, normal_bump_x.x);
